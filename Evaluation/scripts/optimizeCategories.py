...
if __name__ == '__main__':
  ...
  h_DNN_datamix_SR_weighted = inFile.Get("h_DNN_bkg_SR_weighted")
  h_DNN_datamix_SB_weighted = inFile.Get("h_DNN_bkg_SB_weighted")
  h_DNN_data_SB = inFile.Get("h_DNN_data_SB")
  if smooth==True: 
    h_DNN_datamix_SR_weighted_smooth = inFile.Get("h_DNN_bkg_SR_weighted_smooth")
    h_DNN_datamix_SB_weighted_smooth = inFile.Get("h_DNN_bkg_SB_weighted_smooth") 
    h_DNN_data_SB_smooth = inFile.Get("h_DNN_data_SB_smooth") 
  else:
    h_DNN_datamix_SR_weighted_smooth = inFile.Get("h_DNN_bkg_SR_weighted")
    h_DNN_datamix_SB_weighted_smooth = inFile.Get("h_DNN_bkg_SB_weighted") 
  ...
  if nCats == 1:
   ...
   for i in range(1,nBins+1):
       partition = [[i,nBins]]
       significance = sumSignificance(partition, h_DNN_signal_SR, h_DNN_datamix_SR_weighted_smooth, h_DNN_datamix_SB_weighted_smooth, h_DNN_data_SB_smooth, h_DNN_data_SB)
  ...
  for pair in partition_final:
    ...
    b = h_DNN_datamix_SR_weighted_smooth.Integral(pair[0],pair[1])
    m = h_DNN_datamix_SB_weighted_smooth.Integral(pair[0],pair[1])
